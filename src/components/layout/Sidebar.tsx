import React from 'react';
import { Link, useLocation } from 'react-router-dom';
import { useAuth } from '../../contexts/AuthContext';
import './Sidebar.css';

// Embedded logo as base64 - works in both dev and production
const LOGO_BASE64 = 'data:image/png;base64,';

const coreItems = [
    { path: '/dashboard', label: 'Dashboard', icon: '📊' },
    { path: '/users', label: 'Users', icon: '👥' },
    { path: '/products', label: 'Products', icon: '📦' },
    { path: '/clients', label: 'Clients', icon: '🏢' },
    { path: '/projects', label: 'Projects', icon: '🗂️' },
    { path: '/issues', label: 'Issues', icon: '🐛' },
    { path: '/analytics', label: 'Analytics', icon: '📈' },
    { path: '/performance', label: 'Performance', icon: '⚡' },
    { path: '/ml-insights', label: 'ML Insights', icon: '🤖' },
];

const settingsItems = [
    { path: '/email-settings', label: 'Email Settings', icon: '📧' },
    { path: '/email-scheduler', label: 'Email Scheduler', icon: '📅' },
    { path: '/settings', label: 'Settings', icon: '⚙️' },
];

const governanceItems = [
    { path: '/client-health', label: 'Client Health', icon: '🏥' },
    { path: '/shared-issues', label: 'Shared Issues', icon: '🔗' },
    { path: '/office-visits', label: 'Office Visits', icon: '🗃️' },
    { path: '/relationship-resets', label: 'Rel. Resets', icon: '🔄' },
    { path: '/mbr', label: 'Monthly Review', icon: '📑' },
    { path: '/monitoring', label: 'Monitoring', icon: '🔍' },
    { path: '/feature-roadmap', label: 'Feature Roadmap', icon: '🛣️' },
];

const Sidebar: React.FC = () => {
    const location = useLocation();
    const { user, logout } = useAuth();
    const [isCoreExpanded, setIsCoreExpanded] = React.useState(true);
    const [isGovernanceExpanded, setIsGovernanceExpanded] = React.useState(true);
    const [isSettingsExpanded, setIsSettingsExpanded] = React.useState(true);

    const navLink = (item: { path: string; label: string; icon: string }) => (
        <Link
            key={item.path}
            to={item.path}
            className={`sidebar-item ${location.pathname === item.path ? 'active' : ''}`}
        >
            <span className="sidebar-item-icon">{item.icon}</span>
            <span className="sidebar-item-label">{item.label}</span>
        </Link>
    );

    return (
        <aside className="sidebar">
            <div className="sidebar-header">
                <img src={LOGO_BASE64} alt="DevPulse" className="sidebar-logo-icon" />
                <span className="sidebar-logo-text">DevPulse</span>
            </div>

            <nav className="sidebar-nav">
                <div
                    className="sidebar-section-header"
                    onClick={() => setIsCoreExpanded(!isCoreExpanded)}
                >
                    <span>Developer's Insight</span>
                    <span className={`chevron ${isCoreExpanded ? 'expanded' : ''}`}>▼</span>
                </div>
                {isCoreExpanded && (
                    <div className="sidebar-section-content">
                        {coreItems.map(navLink)}
                    </div>
                )}

                <div
                    className="sidebar-section-header"
                    onClick={() => setIsGovernanceExpanded(!isGovernanceExpanded)}
                >
                    <span>Client Governance</span>
                    <span className={`chevron ${isGovernanceExpanded ? 'expanded' : ''}`}>▼</span>
                </div>
                {isGovernanceExpanded && (
                    <div className="sidebar-section-content">
                        {governanceItems.map(navLink)}
                    </div>
                )}

                <div
                    className="sidebar-section-header"
                    onClick={() => setIsSettingsExpanded(!isSettingsExpanded)}
                >
                    <span>Settings & Config</span>
                    <span className={`chevron ${isSettingsExpanded ? 'expanded' : ''}`}>▼</span>
                </div>
                {isSettingsExpanded && (
                    <div className="sidebar-section-content">
                        {settingsItems.map(navLink)}
                    </div>
                )}
            </nav>

            {/* User panel */}
            {user && (
                <div style={{
                    padding: '12px 14px',
                    borderTop: '1px solid rgba(255,255,255,0.07)',
                    marginTop: 'auto',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 10,
                }}>
                    <div style={{
                        width: 32, height: 32,
                        borderRadius: '50%',
                        background: 'linear-gradient(135deg, #6366f1, #4f46e5)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: '0.85rem', fontWeight: 700, color: '#fff', flexShrink: 0,
                    }}>
                        {user.fullName.charAt(0)}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--color-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {user.fullName}
                        </div>
                        <div style={{ fontSize: '0.7rem', color: 'var(--color-text-secondary)', textTransform: 'capitalize' }}>
                            {user.role ?? 'Manager'}
                        </div>
                    </div>
                    <button
                        title="Logout"
                        onClick={logout}
                        style={{
                            background: 'none', border: 'none', cursor: 'pointer',
                            color: 'var(--color-text-secondary)', fontSize: '1rem',
                            padding: '4px', borderRadius: 6, flexShrink: 0,
                            transition: 'color 0.2s',
                        }}
                        onMouseEnter={e => (e.currentTarget.style.color = '#ef4444')}
                        onMouseLeave={e => (e.currentTarget.style.color = 'var(--color-text-secondary)')}
                    >
                        ⏻
                    </button>
                </div>
            )}
        </aside>
    );
};

export default Sidebar;
